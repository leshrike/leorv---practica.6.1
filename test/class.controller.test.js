...
describe('Testing addProductToStore', () => {
	...
	test('llama a la vista con un producto válido', () => {
		const data = {
			name: 'Nombre',
			price: 100
		}
	...
	test('llama al modelo y a la vista con un producto NO válido', () => {
		const data = {
			name: 'Nombre',
			units: 100
		}
		const newProd = new Product(1, 'Nombre', 100, 0);
		myController.addProductToStore(data);
...
describe('Testing changeProductStock', () => {
	...
	test('llama a la vista con una ID válida y con suficientes uds', () => {
		...
		myController.changeProductStock(dataId, units);
	
		expect(myController.view.renderChangeStock.mock.calls.length).toBe(1);
		expect(myController.view.renderChangeStock.mock.calls[0][0]).toEqual(newProd);
	...
	test('llama a la vista con una ID válida sin suficientes uds', () => {
		...
		expect(myController.view.renderChangeStock.mock.calls.length).toBe(0);
		expect(myController.view.renderErrorMessage.mock.calls.length).toBe(1);
		expect(myController.view.renderErrorMessage.mock.calls[0][0]).toBeTruthy();
	})
	...
	test('llama a la vista con una ID NO válida', () => {
		...
		myController.changeProductStock(dataId);
